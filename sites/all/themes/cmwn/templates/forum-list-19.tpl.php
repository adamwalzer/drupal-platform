...
foreach ($forums as $forum) {
  if (in_array($forum_id, $forum->parents)) {
    $children[$forum->tid] = 0;
  } else {
    if (isset($children[$forum->parents[0]])) {
      $children[$forum->parents[0]]++;
    } else {
      if (in_array($forum_id, $forum->parents)) {
        $children[$forum->tid] = 0;
      }
...
foreach ($children as $id => $projects) {
  $forums[$id]->projects = $projects;
}
?>
<table id="forum-<?php print $forum_id; ?>">
  ...
  <?php foreach ($forums as $child_id => $forum): ?>
    <tr id="forum-list-<?php print $child_id; ?>" class="<?php print $forum->zebra.(isset($forum->projects) ? ' interest-group' : ' project'); ?>">
